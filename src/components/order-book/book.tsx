...
const Row = memo(function Row({ px, sz, total, maxTotal, side, szDecimals, flash }: RowProps) {
...
export default function Book({ bids, asks, spread: apiSpread, asset, group, coin }: BookProps) {
  ...
  const { askDisplaySizes, askTotals, bidDisplaySizes, bidTotals, maxTotal } = useMemo(() => {
  ...
  useEffect(() => {
    ...
    for (const level of [...topAsks, ...topBids]) {
      ...
      if (prevSizesRef.current.has(level.px) && prevSizesRef.current.get(level.px) !== curr) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-3 text-sm text-[#a7a7b7] px-3 py-[5px] bg-white/[0.03]">
        <span>Spread</span>
        <span className="text-center">{spread > 0 ? fmt(spread, spreadDecimals) : '—'}</span>
